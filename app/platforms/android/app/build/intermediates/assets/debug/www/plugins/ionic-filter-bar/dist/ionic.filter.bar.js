...
(function (angular, document) {
  ...
  angular.module('jett.ionic.filter.bar')
    .directive('ionFilterBar', [
      ...
      '$ionicPlatform',
      '$state',
      function ($timeout, $ionicGesture, $ionicPlatform, $state) {
            ...
            '<div class="filter-bar-wrapper filter-bar-stable filter-bar-transition-vertical filter-bar-in">' +
              '<div class="bar bar-header bar-stable item-input-inset">' +
                  ...
                  '<div class="row">'+
                    ...
                    '</div>'+
                    '<div class="col-70 item-input-wrapper" style="display:flex !important;">'+
                      '<input type="search" class="filter-bar-search" ng-model="data.filterText" placeholder="{{::config.placeholder}}" />' +
                    '</div>'+
                    '<div class="col-10 item-input-wrapper" style="display:flex !important;">'+
                      '<i style="float:right;" class="filter-bar-clear button button-icon icon_set_1_icon-77" ng-class="getClearButtonClass()"></i>' +
                    '</div>'+
                  '</div>'+
          ...
          filterBarTemplate =
            '<div class="filter-bar-wrapper filter-bar-{{::config.theme}} filter-bar-transition-{{::config.transition}}">' +
              '<div class="bar bar-header bar-{{::config.theme}} item-input-inset">' +
                '<button class="filter-bar-cancel button button-icon icon {{::config.back}}" ui-sref="app.editorial"></button>' +
                '<label class="item-input-wrapper">' +
                  '<input type="search" class="filter-bar-search" ng-model="data.filterText" placeholder="{{::config.placeholder}}" />' +
                  '<button class="filter-bar-clear button button-icon icon_set_1_icon-77" ng-class="getClearButtonClass()"></button>' +
                '</label>' +
              '</div>' +
            '</div>';
            */
      ...
      function ($document, $rootScope, $compile, $timeout, $filter, $ionicPlatform, $ionicFilterBarConfig, $ionicConfig, $ionicModal, $ionicScrollDelegate) {
        ...
        function filterBar (opts) {
          ...
          scope.filterItems = function(filterText) {
            var filterExp, filteredItems, filteredItems2;
            ...
            if (!filterText.length) {
              ...
              //filteredItems2 = scope.filter(scope.items2, filterExp, scope.comparator);
            }

            $timeout(function() {
              scope.update(filteredItems, filterText);
              //scope.update2(filteredItems2, filterText);
              scope.scrollItemsTop();
            });
          };
